...
(function($) {
    ...
    function counter(event) {
        ...
        $('#counter').php(item+"<sup>/<span></span>"+items+"</sup>");
    ...
    $('.lawFirm--contact .nav.nav-pills .nav-item .dropdown-menu .nav-link').on('click', function(){
        var oText = $(this).text();
        $('.lawFirm--contact .nav.nav-pills .nav-item > .nav-link').php(oText + " Office");
    });
    ...
    $('.lawFirm--contact-form').on('submit', 'form', function(e) {
        ...
        $.post($el.attr('action'), $el.serialize(), function(res){
            res = $.parseJSON( res );
            $el.parent('.lawFirm--contact-form').find('.form-response').php('<span>' + res[1] + '</span>');
        });
    });
